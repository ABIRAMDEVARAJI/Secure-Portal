...
function AdminPage() {
  const [items, setItems] = useState([]); const [error, setError] = useState(''); const load = () => api('/api/content').then(setItems).catch((err) => setError(err.message))
  useEffect(load, [])
  ...
  return <main className="page"><div className="admin-heading"><div><p className="eyebrow">ADMIN DESK</p><h1>Keep the library useful.</h1><p className="lede">Publish, refine, and retire internal material.</p></div><Link className="primary-button" to="/admin/upload">＋ Upload content</Link></div>{error && <Notice message={error} />}<div className="admin-list">{items.map((item) => <div className="admin-row" key={item.id}><span className={`type-dot ${item.content_type.toLowerCase()}`} /><div><strong>{item.title}</strong><small>{item.category || 'General'} · {item.content_type}</small></div><div className="row-actions"><Link to={`/content/${item.id}`}>View</Link><Link to={`/admin/content/${item.id}/edit`}>Edit</Link><button onClick={() => remove(item)}>Delete</button></div></div>)}</div></main>
}
